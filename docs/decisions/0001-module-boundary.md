# ADR-001: Keep recipes as an Independent Gnalloy Module
...
`recipes` is published as `gnalloy.org/recipes` and owns this responsibility: Reusable Gnalloy pipeline assembly recipes for TCP, HTTP/1, HTTP/2, HTTP/3, WebSocket, and MQTT.
